...
function sendResponse (done) {
  ...
  if (this.user !== null && this.grantType === 'password') {
    response.user_id = this.user._id.toString()
  }

  if (this.client.clientSecret !== undefined && this.config.accessTokenLifetime !== null && this.refreshToken) response.refresh_token = this.refreshToken;
